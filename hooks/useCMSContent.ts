...
export function useCMSContent<T extends Record<string, unknown> = Record<string, unknown>>(
  ...
  // Helper: Fix a Vite-era image path to work in Next.js, or return null if unfixable
  const fixImagePath = (value: string): string | null => {
    // Cloudinary URLs are always valid
    if (value.includes('cloudinary.com') || value.includes('res.cloudinary')) return value;
    // External URLs are valid
    if (value.startsWith('http://') || value.startsWith('https://')) return value;
    // Data URIs are valid
    if (value.startsWith('data:')) return value;
    // FIX: Vite-style /src/assets/ paths → /assets/ (Next.js serves from /public/)
    if (value.startsWith('/src/assets/')) return value.replace('/src/assets/', '/assets/');
    if (value.startsWith('src/assets/')) return '/' + value.replace('src/assets/', 'assets/');
    // REJECT: Vite production hashed paths (contain hash like -CanACH_M)
    if (value.startsWith('/assets/') && /-[A-Za-z0-9_]{6,}\.(jpg|jpeg|png|gif|webp|svg)$/i.test(value)) return null;
    // Valid: /assets/filename.jpg (served from /public/assets/)
    if (value.startsWith('/assets/')) return value;
    // Valid: /uploads/ or other absolute paths
    if (value.startsWith('/')) return value;
    // REJECT: bare filenames without path context
    return null;
  };

  // Helper: Check if a value is a valid image path (wrapper around fixImagePath)
  const isValidImagePath = (value: unknown): boolean => {
    if (typeof value !== 'string' || !value) return false;
    return fixImagePath(value) !== null;
  };
  ...
  const deepMergeWithDefaults = (cmsData: Partial<T>): T => {
    ...
    for (const key in cmsData) {
      if (cmsData.hasOwnProperty(key)) {
        ...
        // Special handling for image fields - fix Vite paths or fall back to default
        if (isImageField(key) && typeof cmsValue === 'string') {
          const fixed = fixImagePath(cmsValue);
          if (fixed) {
            result[key] = fixed;
          }
          // else: keep default (don't set result[key])
          continue;
        }
        
        // Special handling for galleryImages array - fix each image path
        if (key === 'galleryImages' && Array.isArray(cmsValue)) {
          const validatedImages = cmsValue.map((img: any, idx: number) => {
            if (img && typeof img.src === 'string') {
              const fixed = fixImagePath(img.src);
              if (fixed) {
                return { ...img, src: fixed };
              }
        ...
        if (Array.isArray(cmsValue) && Array.isArray(defaultValue) && cmsValue.length > 0 && typeof cmsValue[0] === 'object') {
          const validatedArray = cmsValue.map((item: any, idx: number) => {
            ...
            // Fix image paths in each object
            for (const itemKey in item) {
              if (isImageField(itemKey) && typeof item[itemKey] === 'string') {
                const fixed = fixImagePath(item[itemKey]);
                if (fixed) {
                  validatedItem[itemKey] = fixed;
                } else if (defaultItem && defaultItem[itemKey]) {
        ...
        // If both are objects (not arrays), merge recursively with image path fixing
        if (
          cmsValue !== null &&
          ...
          for (const subKey in cmsValue) {
            if ((cmsValue as any).hasOwnProperty(subKey)) {
              const subVal = (cmsValue as any)[subKey];
              if (isImageField(subKey) && typeof subVal === 'string') {
                const fixed = fixImagePath(subVal);
                if (fixed) {
                  merged[subKey] = fixed;
                }
                // else: keep default
