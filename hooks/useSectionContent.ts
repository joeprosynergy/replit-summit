...
function isValidImageUrl(value: unknown): boolean {
  ...
  // REJECT: Vite-style /src/assets/ paths - Next.js serves from /public/, not /src/
  if (trimmed.startsWith('/src/') || trimmed.startsWith('src/')) {
    return false;
  }
  
  // REJECT: Vite production hashed paths (contain hash like -CanACH_M)
  const isHashedPath = trimmed.startsWith('/assets/') && /-[A-Za-z0-9_]{6,}\.(jpg|jpeg|png|gif|webp|svg)$/i.test(trimmed);
  if (isHashedPath) {
    return false;
  }
...
/**
 * Fix a Vite-era image URL to work in Next.js. Returns null if unfixable.
 */
function fixImageUrl(value: string): string | null {
  const trimmed = value.trim();
...
function deepMergeWithDefaults<T>(defaults: T, cmsData: Partial<T>): T {
  ...
  for (const key of Object.keys(cmsData)) {
    ...
    // IMAGE URL NORMALIZATION: For image fields, fix Vite paths or fall back to default
    if (isImageField(key) && typeof cmsValue === 'string') {
      const fixed = fixImageUrl(cmsValue);
      if (fixed) {
        result[key] = fixed;
      }
      // else: keep default (skip this CMS value)
      continue;
    }
    ...
    if (Array.isArray(cmsValue) && Array.isArray(defaultValue)) {
      ...
      const merged = cmsValue.map((cmsItem, index) => {
        ...
        // For image arrays (e.g., galleryImages), fix or fall back to default
        if (isImageArray && typeof cmsItem === 'string') {
          const fixed = fixImageUrl(cmsItem);
...
export function useSectionContent<T extends SectionContent>(
  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      if (matches) {
        const invalidPaths = matches.filter(match => {
          const path = match.slice(1, -1); // Remove quotes
          // Valid: full URLs, data URIs, or absolute paths to /assets/ or /uploads/
          const isValid = 
        ...
        if (invalidPaths.length > 0) {
          console.error("[CMS SAFETY NET] INVALID IMAGE PATHS DETECTED - REJECTING CMS DATA", {
            invalidPaths,
