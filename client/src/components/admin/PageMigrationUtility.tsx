...
async function importPageDefaults(slug: string): Promise<{ defaults: Record<string, unknown> } | null> {
  try {
    ...
    if (slug === 'economy-shed') {
      const { economyShedDefaults } = await import('../../pages/defaults/economyShedDefaults');
      console.log(`[Migration] Successfully imported economy-shed defaults`);
      return { defaults: economyShedDefaults as Record<string, unknown> };
    }
    
    if (slug === 'contact-us') {
      const { contactUsDefaults } = await import('../../pages/defaults/contactUsDefaults');
      console.log(`[Migration] Successfully imported contact-us defaults`);
      return { defaults: contactUsDefaults as Record<string, unknown> };
    }
    
    if (slug === 'about-us') {
      const { aboutUsDefaults } = await import('../../pages/defaults/aboutUsDefaults');
      console.log(`[Migration] Successfully imported about-us defaults`);
      return { defaults: aboutUsDefaults as Record<string, unknown> };
    }
    
    if (slug === 'privacy-policy') {
      const { privacyPolicyDefaults } = await import('../../pages/defaults/privacyPolicyDefaults');
      console.log(`[Migration] Successfully imported privacy-policy defaults`);
      return { defaults: privacyPolicyDefaults as Record<string, unknown> };
    }
    
    if (slug === 'buyers-guide') {
      const { buyersGuideDefaults } = await import('../../pages/defaults/buyersGuideDefaults');
      console.log(`[Migration] Successfully imported buyers-guide defaults`);
      return { defaults: buyersGuideDefaults as Record<string, unknown> };
    }
    
    if (slug === 'financing') {
      const { financingDefaults } = await import('../../pages/defaults/financingDefaults');
      console.log(`[Migration] Successfully imported financing defaults`);
      return { defaults: financingDefaults as Record<string, unknown> };
    }
