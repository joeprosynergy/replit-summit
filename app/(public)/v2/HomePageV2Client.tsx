...
/**
 * Homepage V2 — live A/B variant (served at "/" for ~50% of visitors via
 * middleware, and directly at /v2).
 *
 * Main copy blocks, the hero, and the primary buttons are CMS-editable inline
 * (slug "home-v2", see homeV2Defaults) so the team can edit them from the admin
 * login with no code/AI. Card grids, the stats bar, the Summit Standard cards,
 * the How-It-Works steps, the Service Area section, and the reused arrays
 * (stakes pain points, guide stats/credentials, imagine benefits) stay
 * code-managed. Testimonials come from useEditableTestimonials.
 */
...
import { homeV2Defaults, HomeV2Content } from "@/data/defaults/homeV2Defaults";
...
export default function HomePageV2Client() {
  ...
  return (
    <EditablePageWrapper<HomeV2Content> slug="home-v2" defaultContent={homeV2Defaults}>
      {({ content, isEditMode, updateField }) => (
        <div className="v2-root antialiased bg-[#f8f5f0] text-[#1e3149] pt-28">
                ...
                <div className="lg:col-span-7 px-6 lg:pl-12 xl:pl-20 flex items-center py-16 lg:py-0">
                  <div className="max-w-2xl">
                    ...
                    <InlineEditable
                      ...
                      isEditMode={isEditMode}
                      className="v2-heading text-[44px] sm:text-[56px] lg:text-[72px] leading-[1.05] font-bold text-[#1e3149] whitespace-pre-line"
                    />
                    ...
                    <InlineEditable
                      ...
                      isEditMode={isEditMode}
                      className="mt-6 text-xl sm:text-2xl text-[#4a5568] max-w-lg"
                    />
                    ...
                    <div className="mt-9 flex flex-col sm:flex-row gap-4">
                      ...
                      >
                        ...
                        >
                          <span>{content.heroButton1Text}</span>
          ...
          {/* WE UNDERSTAND — copy editable, pain-point cards code-managed */}
          <section className="max-w-screen-2xl mx-auto py-20 px-6 lg:px-12">
            <div className="max-w-3xl">
              ...
              <InlineEditable
                as="h2"
                value={content.stakesHeading}
                fieldName="stakesHeading"
                onChange={(v) => updateField("stakesHeading", v)}
                isEditMode={isEditMode}
                className="v2-heading text-4xl sm:text-5xl lg:text-6xl tracking-[-2.4px] font-bold mt-2"
              />
              <InlineEditable
                as="p"
                type="textarea"
                value={content.stakesSubheading}
                fieldName="stakesSubheading"
                onChange={(v) => updateField("stakesSubheading", v)}
                isEditMode={isEditMode}
                className="mt-4 text-xl text-[#4a5568]"
              />
            </div>
            ...
            <InlineEditable
              as="p"
              value={content.stakesClosingText}
              fieldName="stakesClosingText"
              onChange={(v) => updateField("stakesClosingText", v)}
              isEditMode={isEditMode}
              className="mt-10 text-xl text-[#b85c3a] font-semibold"
            />
          </section>

          {/* FIND YOUR STYLE — copy editable, cards/filters code-managed */}
          <section id="styles" className="max-w-screen-2xl mx-auto pt-12 pb-14 px-6 lg:px-12">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-10">
              <div>
                <div className="text-[#b85c3a] font-semibold tracking-[2px] text-sm">
                  <InlineEditable
                    as="span"
                    value={content.styleTagline}
                    fieldName="styleTagline"
                    onChange={(v) => updateField("styleTagline", v)}
                    isEditMode={isEditMode}
                  />
                </div>
                <InlineEditable
                  as="h2"
                  value={content.styleHeading}
                  fieldName="styleHeading"
                  onChange={(v) => updateField("styleHeading", v)}
                  isEditMode={isEditMode}
                  className="v2-heading text-4xl sm:text-5xl lg:text-6xl tracking-[-2.4px] font-bold mt-1"
                />
              </div>
              <InlineEditable
                as="p"
                type="textarea"
                value={content.styleDescription}
                fieldName="styleDescription"
                onChange={(v) => updateField("styleDescription", v)}
                isEditMode={isEditMode}
                className="text-xl text-[#4a5568] max-w-md mt-4 md:mt-0 md:text-right"
              />
            </div>
            ...
            <div className="text-center mt-9">
              ...
              >
                <InlineEditable
                  as="span"
                  value={content.styleBottomLinkText}
                  fieldName="styleBottomLinkText"
                  onChange={(v) => updateField("styleBottomLinkText", v)}
                  isEditMode={isEditMode}
                />
              </Link>
            </div>
          ...
          {/* YOUR GUIDE — copy editable, stats/credentials code-managed */}
              ...
              <div className="grid lg:grid-cols-12 gap-12 items-start">
                <div className="lg:col-span-7">
                  ...
                  <InlineEditable
                    as="p"
                    type="textarea"
                    value={content.guideParagraph1}
                    fieldName="guideParagraph1"
                    onChange={(v) => updateField("guideParagraph1", v)}
                    isEditMode={isEditMode}
                    className="mt-6 text-xl text-[#1e3149] font-semibold"
                  />
                  <InlineEditable
                    as="p"
                    type="textarea"
                    value={content.guideParagraph2}
                    fieldName="guideParagraph2"
                    onChange={(v) => updateField("guideParagraph2", v)}
                    isEditMode={isEditMode}
                    className="mt-4 text-lg text-[#4a5568] leading-relaxed"
                  />
                  <InlineEditable
                    as="p"
                    type="textarea"
                    value={content.guideParagraph3}
          ...
          {/* THE SUMMIT STANDARD — copy editable, cards code-managed */}
            ...
            <div className="max-w-screen-2xl mx-auto px-6 lg:px-12">
              <div className="max-w-2xl mb-12">
                <div className="uppercase tracking-[3px] text-[#b85c3a] text-sm font-semibold">
                  <InlineEditable
                    as="span"
                    value={content.standardTagline}
                    fieldName="standardTagline"
                    onChange={(v) => updateField("standardTagline", v)}
                    isEditMode={isEditMode}
                  />
                </div>
                <InlineEditable
                  as="h2"
                  value={content.standardHeading}
                  fieldName="standardHeading"
                  onChange={(v) => updateField("standardHeading", v)}
                  isEditMode={isEditMode}
                  className="v2-heading text-white text-4xl sm:text-5xl lg:text-6xl tracking-[-2.6px] leading-none mt-3 font-bold"
                />
              </div>
          ...
          {/* HOW IT WORKS — copy + CTA editable, steps code-managed */}
          <section id="process" className="max-w-screen-2xl mx-auto py-20 px-6 lg:px-12">
            <div className="text-center mb-12">
              ...
              <InlineEditable
                as="h2"
                value={content.processHeading}
                fieldName="processHeading"
                onChange={(v) => updateField("processHeading", v)}
                isEditMode={isEditMode}
                className="v2-heading text-4xl sm:text-5xl lg:text-6xl tracking-[-2.4px] mt-2 font-bold"
              />
            </div>
            ...
            <div className="text-center mt-12">
              <InlineEditableButton
                text={content.processCtaText}
                href={content.processCtaLink}
                isExternal={content.processCtaOpenInNewTab}
                onTextChange={(v) => updateField("processCtaText", v)}
                onHrefChange={(v) => updateField("processCtaLink", v)}
                onExternalChange={(v) => updateField("processCtaOpenInNewTab", v)}
                isEditMode={isEditMode}
              >
                <a
                  href={content.processCtaLink}
                  target={content.processCtaOpenInNewTab ? "_blank" : undefined}
                  rel={content.processCtaOpenInNewTab ? "noopener noreferrer" : undefined}
                  className="v2-modern-btn inline-flex items-center gap-3 bg-[#b85c3a] text-white font-semibold px-8 h-14 rounded-2xl text-lg"
                >
                  {content.processCtaText}
          ...
          {/* STORIES — copy editable, testimonials from hook */}
              ...
              <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between mb-10">
                <div>
                  <div className="text-[#b85c3a] font-semibold tracking-widest text-sm">
                    <InlineEditable
                      as="span"
                      value={content.storiesTagline}
                      fieldName="storiesTagline"
                      onChange={(v) => updateField("storiesTagline", v)}
                      isEditMode={isEditMode}
                    />
                  </div>
                  <InlineEditable
                    as="h2"
                    value={content.storiesHeading}
                    fieldName="storiesHeading"
                    onChange={(v) => updateField("storiesHeading", v)}
                    isEditMode={isEditMode}
                    className="v2-heading text-4xl sm:text-5xl tracking-[-2px] mt-2 font-bold"
                  />
                </div>
                <Link
                  href="/gallery"
                ...
                >
                  <InlineEditable
                    as="span"
                    value={content.storiesLinkText}
                    fieldName="storiesLinkText"
                    onChange={(v) => updateField("storiesLinkText", v)}
                    isEditMode={isEditMode}
                  />
                </Link>
              </div>
          ...
          {/* IMAGINE THIS — copy editable, benefits code-managed */}
          ...
          {/* FINAL CTA — copy + buttons editable */}
            ...
            <div className="max-w-screen-2xl mx-auto px-6 lg:px-12 text-center">
              <div className="max-w-2xl mx-auto">
                <InlineEditable
                  ...
                  isEditMode={isEditMode}
                  className="v2-heading text-3xl sm:text-4xl lg:text-5xl tracking-[-1.6px] font-bold leading-tight"
                />
                <InlineEditable
                  as="p"
                  type="textarea"
                  value={content.finalCtaSubheading}
                  fieldName="finalCtaSubheading"
                  onChange={(v) => updateField("finalCtaSubheading", v)}
                  isEditMode={isEditMode}
                ...
                <div className="mt-8 flex flex-col sm:flex-row gap-4 justify-center">
                  <InlineEditableButton
                    text={content.finalCtaPhoneText}
                    href={content.finalCtaPhoneLink}
                    onTextChange={(v) => updateField("finalCtaPhoneText", v)}
                    onHrefChange={(v) => updateField("finalCtaPhoneLink", v)}
                    isEditMode={isEditMode}
                  >
                    <a
                      href={content.finalCtaPhoneLink}
                    ...
                    >
                      ...
                      {content.finalCtaPhoneText}
                    </a>
                  </InlineEditableButton>
                  <InlineEditableButton
                    text={content.finalCtaConfigText}
                    href={content.finalCtaConfigLink}
                    isExternal={content.finalCtaConfigOpenInNewTab}
                    onTextChange={(v) => updateField("finalCtaConfigText", v)}
                    onHrefChange={(v) => updateField("finalCtaConfigLink", v)}
                    onExternalChange={(v) => updateField("finalCtaConfigOpenInNewTab", v)}
                    isEditMode={isEditMode}
                  >
                    <a
                      href={content.finalCtaConfigLink}
                      target={content.finalCtaConfigOpenInNewTab ? "_blank" : undefined}
                      rel={content.finalCtaConfigOpenInNewTab ? "noopener noreferrer" : undefined}
                ...
                <InlineEditable
                  as="p"
                  value={content.finalCtaFinePrint}
                  fieldName="finalCtaFinePrint"
                  onChange={(v) => updateField("finalCtaFinePrint", v)}
                  isEditMode={isEditMode}
                  className="mt-6 text-xs text-white/50 tracking-wider"
                />
              </div>
            </div>
          </div>
