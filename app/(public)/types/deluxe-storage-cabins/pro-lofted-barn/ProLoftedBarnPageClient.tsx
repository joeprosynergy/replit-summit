...
interface ProLoftedBarnPageClientProps {
  initialContent: any;
  config: ProductPageConfig;
}

export default function ProLoftedBarnPageClient({ initialContent, config }: ProLoftedBarnPageClientProps) {
  const { isAdmin } = useOptionalAdminAuth();
  const { content, isLoading } = useCMSContent("pro-lofted-barn", initialContent);
