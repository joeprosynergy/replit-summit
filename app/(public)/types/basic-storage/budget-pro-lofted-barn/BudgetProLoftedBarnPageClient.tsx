...
interface BudgetProLoftedBarnPageClientProps {
  initialContent: any;
  config: ProductPageConfig;
}

export default function BudgetProLoftedBarnPageClient({ initialContent, config }: BudgetProLoftedBarnPageClientProps) {
  const { isAdmin } = useOptionalAdminAuth();
  const { content, isLoading } = useCMSContent("budget-pro-lofted-barn", initialContent);
