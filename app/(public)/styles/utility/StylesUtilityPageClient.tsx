...
/**
 * StylesUtilityPageClient - Next.js Client Component
...
import { stylesUtilityDefaults, StylesUtilityContent, OptionItem, OtherStyleItem } from '@/data/defaults/stylesUtilityDefaults';

export default function StylesUtilityPageClient({ initialContent }: { initialContent: any }) {
  const currentPath = usePathname();

  const renderOptionCard = (
    option: OptionItem,
    index: number,
    content: StylesUtilityContent,
    isEditMode: boolean,
    updateField: <K extends keyof StylesUtilityContent>(field: K, value: StylesUtilityContent[K]) => void,
    fieldName: 'storageOptions' | 'cabinOptions' | 'garageOptions'
  ) => {
    const updateOption = (field: keyof OptionItem, value: string | boolean | string[]) => {
      ...
      updateField(fieldName, options as unknown as StylesUtilityContent[typeof fieldName]);
    };
  ...
  return (
    <EditablePageWrapper<StylesUtilityContent>
      slug="styles-utility"
      defaultContent={stylesUtilityDefaults}
            ...
            <section className="bg-background py-16 md:py-24">
              <div className="container-custom">
                <div className="bg-card rounded-lg shadow-lg overflow-hidden">
                  <div className="bg-primary py-5 px-6">
                    <InlineEditable
                      value={content.garagesHeader}
                      fieldName="garagesHeader"
                      onChange={(v) => updateField('garagesHeader', v)}
                      isEditMode={isEditMode}
                      className="text-2xl md:text-3xl font-heading font-bold text-primary-foreground text-center uppercase tracking-wide"
                      as="h2"
                    />
                  ...
                  <div className="p-6 md:p-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-2xl mx-auto">
                      {content.garageOptions.map((option, index) => 
                        renderOptionCard(option, index, content, isEditMode, updateField, 'garageOptions')
                      )}
                    </div>
                  </div>
                </div>
